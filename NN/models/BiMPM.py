...
class bimpm(Base):
	def __init__(self, args):
		...
		for i in range(1, 9):
			setattr(self, f'mp_w{i}',
					nn.Parameter(torch.rand(self.l, self.hidden_dim)))
	...
	def forward(self, querys,lengths,label=None):
		...
		p = self.word_emb(querys[0])
		h = self.word_emb(querys[1])
		p = self.dropout(p)
		h = self.dropout(h)
		...
		x = torch.cat(
			[agg_p_last.permute(1, 0, 2).contiguous().view(-1, self.hidden_dim * 2),
			 agg_h_last.permute(1, 0, 2).contiguous().view(-1, self.hidden_dim * 2)], dim=1)
		x = self.dropout(x)
		# ----- Prediction Layer -----
